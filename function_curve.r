...
mycast<-function(biom,step,c0,c1,y0,y1){
  ...
  # definisco le classi
  my.lim<-seq(1-step,5+step,by=step)
...
my.plot<-function(x,y){
  ...
  {y<-y/max(y)
   ...
   plot(r,pch=15,lty=1,add=T,lwd=1.5, col="red") #curva
   ...
   abline(v=infl,col="green",lty=3,lwd=3)
   abline(a=int,b=sl,col="red",lty=3,lwd=3)
   abline(h=bio,col="pink",lty=3,lwd=3)
   abline(h=coef(r)[3],col="orange",lty=3,lwd=3)
